...
func (c *BitcoindClient) GetBlockVerbose(
	hash *chainhash.Hash) (*paytjson.GetBlockVerboseResult, error) {
...
func (c *BitcoindClient) GetBlockHeaderVerbose(
	hash *chainhash.Hash) (*paytjson.GetBlockHeaderVerboseResult, error) {
...
func (c *BitcoindClient) GetRawTransactionVerbose(
	hash *chainhash.Hash) (*paytjson.TxRawResult, error) {
...
func (c *BitcoindClient) GetTxOut(txHash *chainhash.Hash, index uint32,
	mempool bool) (*paytjson.GetTxOutResult, error) {
...
//
// NOTE: This is part of the chain.Interface interface.
func (c *BitcoindClient) NotifyReceived(addrs []paytutil.Address) error {
	c.NotifyBlocks()
...
//	[]wire.OutPoint
//	[]*wire.OutPoint
//	map[wire.OutPoint]paytutil.Address
//	[]chainhash.Hash
//	[]*chainhash.Hash
func (c *BitcoindClient) LoadTxFilter(reset bool, filters ...interface{}) error {
	...
	for _, filter := range filters {
		...
		case []paytutil.Address, []wire.OutPoint, []*wire.OutPoint,
			map[wire.OutPoint]paytutil.Address, []chainhash.Hash,
			[]*chainhash.Hash:
...
// matched as []paytjson.BlockDetails.
func (c *BitcoindClient) RescanBlocks(
	blockHashes []chainhash.Hash) ([]paytjson.RescannedBlock, error) {

	rescannedBlocks := make([]paytjson.RescannedBlock, 0, len(blockHashes))
	for _, hash := range blockHashes {
		...
		if len(relevantTxs) > 0 {
			rescannedBlock := paytjson.RescannedBlock{
				Hash: hash.String(),
			}
...
func (c *BitcoindClient) Rescan(blockHash *chainhash.Hash,
	addresses []paytutil.Address, outPoints map[wire.OutPoint]paytutil.Address) error {
...
func (c *BitcoindClient) rescanHandler() {
	...
	for {
		...
		case update := <-c.rescanUpdate:
			...
			case map[wire.OutPoint]paytutil.Address:
				c.watchMtx.Lock()
				for op := range update {
					c.watchedOutPoints[op] = struct{}{}
				}
...
func (c *BitcoindClient) onRelevantTx(tx *wtxmgr.TxRecord,
	blockDetails *paytjson.BlockDetails) {
...
func (c *BitcoindClient) rescan(start chainhash.Hash) error {
	...
	for i := lastHeader.Height + 1; i <= bestBlock.Height; i++ {
		...
		for _, hash := range hashes {
			...
			lastHash = &blockHash
			lastHeader = &paytjson.GetBlockHeaderVerboseResult{
