...
// operate on the same payt network as described by the passed chain
...
func (c *RPCClient) Rescan(startHash *chainhash.Hash, addrs []paytutil.Address,
	outPoints map[wire.OutPoint]paytutil.Address) error {
...
// payt RPC server.  This channel must be continually read or the process
...
// parseBlock parses a paytws definition of the block a tx is mined it to the
...
func parseBlock(block *paytjson.BlockDetails) (*wtxmgr.BlockMeta, error) {
	if block == nil {
		return nil, nil
	}
...
func (c *RPCClient) onRecvTx(tx *paytutil.Tx, block *paytjson.BlockDetails) {
...
func (c *RPCClient) onRedeemingTx(tx *paytutil.Tx, block *paytjson.BlockDetails) {
