...
var (
	walletDataDirectory = paytutil.AppDataDir("paytwallet", false)
	newlineBytes        = []byte{'\n'}
)
...
// Flags.
var opts = struct {
	TestNet1              bool                `long:"testnet" description:"Use the test payt network (version 1)"`
	SimNet                bool                `long:"simnet" description:"Use the simulation payt network"`
...
func init() {
	...
	if opts.TestNet1 && opts.SimNet {
		fatalf("Multiple payt networks may not be used simultaneously")
	}
...
func makeInputSource(outputs []paytjson.ListUnspentResult) txauthor.InputSource {
	var (
		totalInputValue paytutil.Amount
		inputs          = make([]*wire.TxIn, 0, len(outputs))
		inputValues     = make([]paytutil.Amount, 0, len(outputs))
		sourceErr       error
	)
	for _, output := range outputs {
		outputAmount, err := paytutil.NewAmount(output.Amount)
		if err != nil {
	...
	return func(paytutil.Amount) (paytutil.Amount, []*wire.TxIn, []paytutil.Amount, [][]byte, error) {
...
func sweep() error {
	...
	sourceOutputs := make(map[string][]paytjson.ListUnspentResult)
	...
	var totalSwept paytutil.Amount
	var numErrors int
	...
	for _, previousOutputs := range sourceOutputs {
		...
		outputAmount := paytutil.Amount(tx.Tx.TxOut[0].Value)
...
func saneOutputValue(amount paytutil.Amount) bool {
	return amount >= 0 && amount <= paytutil.MaxNanopayt
}

func parseOutPoint(input *paytjson.ListUnspentResult) (wire.OutPoint, error) {
