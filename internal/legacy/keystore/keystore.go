...
import (
	...
	"github.com/paytia-dag/paytwallet/internal/legacy/rename"
)
...
func pubkeyFromPrivkey(privkey []byte, compress bool) (pubkey []byte) {
	_, pk := payt.PrivKeyFromBytes(payt.S256(), privkey)
...
func chainedPrivKey(privkey, pubkey, chaincode []byte) ([]byte, error) {
	...
	case payt.PubKeyBytesLenUncompressed, payt.PubKeyBytesLenCompressed:
		// Correct length
	default:
...
func chainedPubKey(pubkey, chaincode []byte) ([]byte, error) {
	var compressed bool
	switch n := len(pubkey); n {
	case payt.PubKeyBytesLenUncompressed:
		compressed = false
	case payt.PubKeyBytesLenCompressed:
		compressed = true
	default:
	...
	oldPk, err := payt.ParsePubKey(pubkey, payt.S256())
	if err != nil {
		return nil, err
	}
	newX, newY := payt.S256().ScalarMult(oldPk.X, oldPk.Y, xorbytes)
	if err != nil {
		return nil, err
	}
	newPk := &payt.PublicKey{
		Curve: payt.S256(),
		X:     newX,
		Y:     newY,
	}
...
func (net *netParams) ReadFrom(r io.Reader) (int64, error) {
	...
	switch wire.paytNet(binary.LittleEndian.Uint32(uint32Bytes)) {
	case wire.MainNet:
...
func getAddressKey(addr paytutil.Address) addressKey {
...
type Store struct {
	...
	passphrase       []byte
	secret           []byte
	chainIdxMap      map[int64]paytutil.Address
...
func New(dir string, desc string, passphrase []byte, net *chaincfg.Params,
	...
	s := &Store{
		...
		chainIdxMap:      make(map[int64]paytutil.Address),
	...
	root, err := newRootpaytAddress(s, rootkey, nil, chaincode,
		createdAt)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ReadFrom(r io.Reader) (n int64, err error) {
	...
	s.chainIdxMap = make(map[int64]paytutil.Address)
...
func (s *Store) writeTo(w io.Writer) (n int64, err error) {
	...
	for _, wAddr := range s.addrMap {
		switch paytAddr := wAddr.(type) {
		case *paytAddress:
			e := &addrEntry{
				addr: *paytAddr,
			}
			copy(e.pubKeyHash160[:], paytAddr.AddrHash())
			if paytAddr.Imported() {
				// No order for imported addresses.
				importedAddrs = append(importedAddrs, e)
			} else if paytAddr.chainIndex >= 0 {
				// Chained addresses are sorted.  This is
				// kind of nice but probably isn't necessary.
				chainedAddrs[paytAddr.chainIndex] = e
			}
		...
		case *scriptAddress:
			...
			copy(e.scriptHash160[:], paytAddr.AddrHash())
...
func (s *Store) Lock() (err error) {
	...
	for _, addr := range s.addrMap {
		if baddr, ok := addr.(*paytAddress); ok {
			_ = baddr.lock()
		}
	}
...
func (s *Store) ChangePassphrase(new []byte) error {
	...
	for _, wa := range s.addrMap {
		// Only paytAddresses currently have private keys.
		a, ok := wa.(*paytAddress)
		if !ok {
			continue
		}
...
func (s *Store) NextChainedAddress(bs *BlockStamp) (paytutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedAddress(bs *BlockStamp) (paytutil.Address, error) {
	addr, err := s.nextChainedpaytAddress(bs)
	if err != nil {
		return nil, err
	}
...
func (s *Store) ChangeAddress(bs *BlockStamp) (paytutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) nextChainedpaytAddress(bs *BlockStamp) (*paytAddress, error) {
...
func (s *Store) LastChainedAddress() paytutil.Address {
	s.mtx.RLock()
	defer s.mtx.RUnlock()
...
func (s *Store) extendUnlocked(bs *BlockStamp) error {
	...
	newAddr, err := newpaytAddress(s, privkey, nil, bs, true)
	if err != nil {
		return err
	}
...
func (s *Store) extendLocked(bs *BlockStamp) error {
	...
	newaddr, err := newpaytAddressWithoutPrivkey(s, nextPubkey, nil, bs)
	if err != nil {
		return err
	}
	a = newaddr.Address()
...
func (s *Store) createMissingPrivateKeys() error {
	...
	prevAddr, ok := prevWAddr.(*paytAddress)
	if !ok {
		return errors.New("found non-pubkey chained address")
	}
...
func (s *Store) Address(a paytutil.Address) (WalletAddress, error) {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	// Look up address by address hash.
	paytAddr, ok := s.addrMap[getAddressKey(a)]
	if !ok {
...
// Net returns the payt network parameters for this key store.
...
func (s *Store) SetSyncStatus(a paytutil.Address, ss SyncStatus) error {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (s *Store) ImportPrivateKey(wif *paytutil.WIF, bs *BlockStamp) (paytutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	pkh := paytutil.Hash160(wif.SerializePubKey())
	...
	paytAddr, err := newpaytAddress(s, privKey, nil, bs, wif.CompressPubKey)
	if err != nil {
		return nil, err
	}
	paytAddr.chainIndex = importedKeyChainIdx

	// Mark as unsynced if import height is below currently-synced
	// height.
	if len(s.recent.hashes) != 0 && bs.Height < s.recent.lastHeight {
		paytAddr.flags.unsynced = true
	}

	// Encrypt imported address with the derived AES key.
	if err = paytAddr.encrypt(s.secret); err != nil {
		return nil, err
	}

	addr := paytAddr.Address()
	...
	s.addrMap[getAddressKey(addr)] = paytAddr
	s.importedAddrs = append(s.importedAddrs, paytAddr)
...
func (s *Store) ImportScript(script []byte, bs *BlockStamp) (paytutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
	...
	if _, ok := s.addrMap[addressKey(paytutil.Hash160(script))]; ok {
		return nil, ErrDuplicate
	}
...
func (s *Store) ExportWatchingWallet() (*Store, error) {
	...
	ws.keyGenerator = *(kgwc.(*paytAddress))
	...
	for apkh, addr := range s.addrMap {
		if !addr.Imported() {
			// Must be a paytAddress if !imported.
			paytAddr := addr.(*paytAddress)

			ws.chainIdxMap[paytAddr.chainIndex] =
				addr.Address()
		}
		apkhCopy := apkh
...
type WalletAddress interface {
	// Address returns a paytutil.Address for the backing address.
	Address() paytutil.Address
...
func (s *Store) ActiveAddresses() map[paytutil.Address]WalletAddress {
	s.mtx.RLock()
	defer s.mtx.RUnlock()

	addrs := make(map[paytutil.Address]WalletAddress)
	for i := int64(rootKeyChainIdx); i <= s.highestUsed; i++ {
		a := s.chainIdxMap[i]
...
//
// A slice is returned with the paytutil.Address of each new address.
// The blockchain must be rescanned for these addresses.
func (s *Store) ExtendActiveAddresses(n int) ([]paytutil.Address, error) {
	s.mtx.Lock()
	defer s.mtx.Unlock()
...
func (af *addrFlags) ReadFrom(r io.Reader) (int64, error) {
	...
	// paytwallet shall refuse to open any unencrypted addresses.  This
...
// that paytwallet fits into an unused space left by Armory's key store file
// format.
...
type PubKeyAddress interface {
	...
	PrivKey() (*payt.PrivateKey, error)
	// ExportPrivKey exports the WIF private key.
	ExportPrivKey() (*paytutil.WIF, error)
}

// newpaytAddress initializes and returns a new address.  privkey must
// be 32 bytes.  iv must be 16 bytes, or nil (in which case it is
// randomly generated).
func newpaytAddress(wallet *Store, privkey, iv []byte, bs *BlockStamp, compressed bool) (addr *paytAddress, err error) {
	if len(privkey) != 32 {
		return nil, errors.New("private key is not 32 bytes")
	}

	addr, err = newpaytAddressWithoutPrivkey(wallet,
...
// newpaytAddressWithoutPrivkey initializes and returns a new address with an
...
// randomly generated).
func newpaytAddressWithoutPrivkey(s *Store, pubkey, iv []byte, bs *BlockStamp) (addr *paytAddress, err error) {
	var compressed bool
	switch n := len(pubkey); n {
	case payt.PubKeyBytesLenCompressed:
		compressed = true
	...
	pk, err := payt.ParsePubKey(pubkey, payt.S256())
	if err != nil {
		return nil, err
	}

	address, err := paytutil.NewAddressPubKeyHash(paytutil.Hash160(pubkey), s.netParams())
	if err != nil {
		return nil, err
	}
...
// newRootpaytAddress generates a new address, also setting the
// chaincode and chain index to represent this address as a root
// address.
func newRootpaytAddress(s *Store, privKey, iv, chaincode []byte,
	bs *BlockStamp) (addr *paytAddress, err error) {

	if len(chaincode) != 32 {
		return nil, errors.New("chaincode is not 32 bytes")
	}

	// Create new paytAddress with provided inputs.  This will
	// always use a compressed pubkey.
	addr, err = newpaytAddress(s, privKey, iv, bs, true)
	if err != nil {
		return nil, err
	}
...
// unlocked paytAddress.
func (a *paytAddress) verifyKeypairs() error {
	if len(a.privKeyCT) != 32 {
		return errors.New("private key unavailable")
	}

	privKey := &payt.PrivateKey{
...
func (a *paytAddress) ReadFrom(r io.Reader) (n int64, err error) {
	var read int64
	...
	addr, err := paytutil.NewAddressPubKeyHash(pubKeyHash[:], a.store.netParams())
	if err != nil {
		return n, err
	}
	a.address = addr
...
func (a *paytAddress) WriteTo(w io.Writer) (n int64, err error) {
	var written int64
...
func (a *paytAddress) encrypt(key []byte) error {
...
func (a *paytAddress) unlock(key []byte) (privKeyCT []byte, err error) {
	if !a.flags.encrypted {
	...
	x, y := payt.S256().ScalarBaseMult(privkey)
...
func (a *paytAddress) changeEncryptionKey(oldkey, newkey []byte) error {
...
func (a *paytAddress) SyncStatus() SyncStatus {
	switch {
...
// PubKeyAddress.
func (a *paytAddress) PubKey() *payt.PublicKey {
	return a.pubKey
}

func (a *paytAddress) pubKeyBytes() []byte {
	if a.Compressed() {
...
func (a *paytAddress) ExportPubKey() string {
...
func (a *paytAddress) PrivKey() (*payt.PrivateKey, error) {
...
func (a *paytAddress) ExportPrivKey() (*paytutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}
	...
	// elsewhere.
	wif, err := paytutil.NewWIF((*payt.PrivateKey)(pk), a.store.netParams(),
		a.Compressed())
	if err != nil {
		panic(err)
	}
...
// normal key store.
func (a *paytAddress) watchingCopy(s *Store) walletAddress {
	return &paytAddress{
		store:   s,
		address: a.address,
		flags: addrFlags{
...
func (a *paytAddress) setSyncStatus(s SyncStatus) {
	switch e := s.(type) {
	case Unsynced:
...
func newScriptAddress(s *Store, script []byte, bs *BlockStamp) (addr *scriptAddress, err error) {
	...
	scriptHash := paytutil.Hash160(script)

	address, err := paytutil.NewAddressScriptHashFromHash(scriptHash, s.netParams())
	if err != nil {
		return nil, err
	}
...
func (sa *scriptAddress) ReadFrom(r io.Reader) (n int64, err error) {
	...
	address, err := paytutil.NewAddressScriptHashFromHash(scriptHash[:],
		sa.store.netParams())
	if err != nil {
		return n, err
	}
...
// address returns a paytutil.AddressScriptHash for a paytAddress.
func (sa *scriptAddress) Address() paytutil.Address {
	return sa.address
}
...
func (sa *scriptAddress) Addresses() []paytutil.Address {
	return sa.addresses
}
