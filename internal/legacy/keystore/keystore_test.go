...
func TestpaytAddressSerializer(t *testing.T) {
	...
	addr, err := newpaytAddress(fakeWallet, privKey, nil,
		makeBS(0), true)
	if err != nil {
		t.Error(err.Error())
		return
	...
	if !reflect.DeepEqual(addr, &readAddr) {
		t.Error("Original and read paytAddress differ.")
	}
}
...
func TestChaining(t *testing.T) {
	...
	for _, test := range tests {
		...
		// Public keys (used to generate the payt address) MUST match.
		...
		// the next pubkeys.
		pubkeyUncompressed, err := payt.ParsePubKey(nextPubUncompressedFromPub, payt.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next uncompressed pubkey: %v", test.name, err)
			return
		}
		pubkeyCompressed, err := payt.ParsePubKey(nextPubCompressedFromPub, payt.S256())
		if err != nil {
			t.Errorf("%s: Unable to parse next compressed pubkey: %v", test.name, err)
			return
		}
		privkeyUncompressed := &payt.PrivateKey{
			PublicKey: *pubkeyUncompressed.ToECDSA(),
			D:         new(big.Int).SetBytes(nextPrivUncompressed),
		}
		privkeyCompressed := &payt.PrivateKey{
		...
		if !ok {
			t.Errorf("%s: payt signature verification failed for next keypair (chained from uncompressed pubkey).",
				test.name)
			return
		}
		...
		if !ok {
			t.Errorf("%s: payt signature verification failed for next keypair (chained from compressed pubkey).",
				test.name)
			return
		}
	}
...
func TestWalletPubkeyChaining(t *testing.T) {
	...
	if !ok {
		t.Errorf("payt signature verification failed; address's pubkey mismatches the privkey.")
		return
	}
	...
	ok = sig.Verify(hash, pubKey)
	if !ok {
		t.Errorf("payt signature verification failed; next address's keypair does not match.")
		return
	}
...
func TestWatchingWalletExport(t *testing.T) {
	...
	pk, _ := payt.PrivKeyFromBytes(payt.S256(), make([]byte, 32))
	wif, err := paytutil.NewWIF(pk, tstNetParams, true)
	if err != nil {
		t.Fatal(err)
	}
...
func TestImportPrivateKey(t *testing.T) {
	...
	pk, err := payt.NewPrivateKey(payt.S256())
	if err != nil {
	...
	// import priv key
	wif, err := paytutil.NewWIF((*payt.PrivateKey)(pk), tstNetParams, false)
	if err != nil {
		t.Fatal(err)
	}
