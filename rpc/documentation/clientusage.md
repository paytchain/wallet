...
auto main() -> int {
    ...
    if (!status.ok()) {
        ...
        std::cout << "Spendable balance: " << response.spendable() << " nanopayt" << std::endl;
    }
}
```
    ...
    static class Program
    {
        ...
        static async Task ExampleAsync()
        {
            ...
            try
            {
                ...
                Console.WriteLine($"Spendable balance: {balanceResponse.Spendable} nanopayt");
            }
            finally
            {
...
client.balance(request, function(err, response) {
    if (err) {
        ...
        console.log('Spendable balance:', response.spendable, 'nanopayt');
    }
});
```
...
def main():
    ...
    print 'Spendable balance: %d nanopayt' % response.spendable
