...
// request into a marshalable response.  If the error is a *paytjson.RPCError
...
// catch-all error code, paytjson.ErrRPCWallet.
...
var rpcHandlers = map[string]struct {
	...
	// Reference methods which can't be implemented by paytwallet due to
...
func unimplemented(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &paytjson.RPCError{
		Code:    paytjson.ErrRPCUnimplemented,
		Message: "Method unimplemented",
	}
}

// unsupported handles a standard bitcoind RPC request which is
// unsupported by paytwallet due to design differences.
func unsupported(interface{}, *wallet.Wallet) (interface{}, error) {
	return nil, &paytjson.RPCError{
		Code:    -1,
		Message: "Request unsupported by paytwallet",
	}
}
...
// context.
type lazyHandler func() (interface{}, *paytjson.RPCError)
...
func lazyApplyHandler(request *paytjson.Request, w *wallet.Wallet, chainClient chain.Interface) lazyHandler {
	handlerData, ok := rpcHandlers[request.Method]
	if ok && handlerData.handlerWithChain != nil && w != nil && chainClient != nil {
		return func() (interface{}, *paytjson.RPCError) {
			cmd, err := paytjson.UnmarshalCmd(request)
			if err != nil {
				return nil, paytjson.ErrRPCInvalidRequest
			}
...
func makeResponse(id, result interface{}, err error) paytjson.Response {
	idPtr := idPointer(id)
	if err != nil {
		return paytjson.Response{
			ID:    idPtr,
			Error: jsonError(err),
		}
	}
	...
	if err != nil {
		return paytjson.Response{
			ID: idPtr,
			Error: &paytjson.RPCError{
				Code:    paytjson.ErrRPCInternal.Code,
...
func jsonError(err error) *paytjson.RPCError {
	if err == nil {
		return nil
	}
	...
	switch e := err.(type) {
	case paytjson.RPCError:
		return &e
	case *paytjson.RPCError:
		return e
	case DeserializationError:
		code = paytjson.ErrRPCDeserialization
	case InvalidParameterError:
		code = paytjson.ErrRPCInvalidParameter
	case ParseError:
		code = paytjson.ErrRPCParse.Code
	case waddrmgr.ManagerError:
		switch e.ErrorCode {
		case waddrmgr.ErrWrongPassphrase:
			code = paytjson.ErrRPCWalletPassphraseIncorrect
		}
	}
	return &paytjson.RPCError{
		Code:    code,
		Message: err.Error(),
	}
}
...
func makeMultiSigScript(w *wallet.Wallet, keys []string, nRequired int) ([]byte, error) {
	keysesPrecious := make([]*paytutil.AddressPubKey, len(keys))
	...
	for i, a := range keys {
		...
		default:
			...
			pubKeyAddr, err := paytutil.NewAddressPubKey(
...
func addMultiSigAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.AddMultisigAddressCmd)
	...
	secp256k1Addrs := make([]paytutil.Address, len(cmd.Keys))
...
func createMultiSig(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.CreateMultisigCmd)
	...
	address, err := paytutil.NewAddressScriptHash(script, w.ChainParams())
	if err != nil {
		// above is a valid script, shouldn't happen.
		return nil, err
	}

	return paytjson.CreateMultiSigResult{
...
func dumpPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.DumpPrivKeyCmd)
...
func getAddressesByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetAddressesByAccountCmd)
...
func getBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetBalanceCmd)

	var balance paytutil.Amount
	var err error
	accountName := "*"
	if cmd.Account != nil {
...
func getBestBlock(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	result := &paytjson.GetBestBlockResult{
...
func decodeAddress(s string, params *chaincfg.Params) (paytutil.Address, error) {
	addr, err := paytutil.DecodeAddress(s, params)
	if err != nil {
		msg := fmt.Sprintf("Invalid address %q: decode failed with %#q", s, err)
		return nil, &paytjson.RPCError{
			Code:    paytjson.ErrRPCInvalidAddressOrKey,
			Message: msg,
		}
	}
...
func getAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetAccountCmd)
...
func getAccountAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetAccountAddressCmd)
...
func getUnconfirmedBalance(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetUnconfirmedBalanceCmd)
...
func importPrivKey(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	wif, err := paytutil.DecodeWIF(cmd.PrivKey)
	if err != nil {
		return nil, &paytjson.RPCError{
			Code:    paytjson.ErrRPCInvalidAddressOrKey,
...
func createNewAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.CreateNewAccountCmd)
...
func renameAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.RenameAccountCmd)
...
func getNewAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetNewAddressCmd)
...
func getRawChangeAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetRawChangeAddressCmd)
...
func getReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetReceivedByAccountCmd)
	...
	return results[acctIndex].TotalReceived.Topayt(), nil
}
...
func getReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.GetReceivedByAddressCmd)
...
func getTransaction(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	ret := paytjson.GetTransactionResult{
		TxID:            cmd.Txid,
	...
	var (
		debitTotal  paytutil.Amount
		creditTotal paytutil.Amount // Excludes change
		fee         paytutil.Amount
		feeF64      float64
	)
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var outputTotal paytutil.Amount
		for _, output := range details.MsgTx.TxOut {
			outputTotal += paytutil.Amount(output.Value)
		}
	...
	if len(details.Debits) == 0 {
		...
		ret.Details = make([]paytjson.GetTransactionDetailsResult, 0, len(details.Credits))
	} else {
		ret.Details = make([]paytjson.GetTransactionDetailsResult, 1, len(details.Credits)+1)

		ret.Details[0] = paytjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Account
			//   Address
			...
			Category: "send",
			Amount:   (-debitTotal).Topayt(), // negative since it is a send
			Fee:      &feeF64,
		}
		ret.Fee = feeF64
	}
	...
	for _, cred := range details.Credits {
		...
		ret.Details = append(ret.Details, paytjson.GetTransactionDetailsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   Fee
			Account:  accountName,
			Address:  address,
			Category: credCat,
			Amount:   cred.Amount.Topayt(),
			Vout:     cred.Index,
		})
	}

	ret.Amount = creditTotal.Topayt()
	return ret, nil
}
...
func listAccounts(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ListAccountsCmd)
	...
	for _, result := range results {
		accountBalances[result.AccountName] = result.AccountBalance.Topayt()
	}
...
func listReceivedByAccount(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ListReceivedByAccountCmd)
	...
	jsonResults := make([]paytjson.ListReceivedByAccountResult, 0, len(results))
	for _, result := range results {
		jsonResults = append(jsonResults, paytjson.ListReceivedByAccountResult{
			Account:       result.AccountName,
			Amount:        result.TotalReceived.Topayt(),
...
func listReceivedByAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ListReceivedByAddressCmd)

	// Intermediate data for each address.
	type AddrData struct {
		// Total amount received.
		amount paytutil.Amount
	...
	ret := make([]paytjson.ListReceivedByAddressResult, numAddresses, numAddresses)
	idx := 0
	for address, addrData := range allAddrData {
		ret[idx] = paytjson.ListReceivedByAddressResult{
			Address:       address,
			Amount:        addrData.amount.Topayt(),
...
func listSinceBlock(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*paytjson.ListSinceBlockCmd)
	...
	res := paytjson.ListSinceBlockResult{
...
func listTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ListTransactionsCmd)
...
func listAddressTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ListAddressTransactionsCmd)
...
func listAllTransactions(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ListAllTransactionsCmd)
...
func listUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ListUnspentCmd)
...
func lockUnspent(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.LockUnspentCmd)
...
// and amounts.
func makeOutputs(pairs map[string]paytutil.Amount, chainParams *chaincfg.Params) ([]*wire.TxOut, error) {
	outputs := make([]*wire.TxOut, 0, len(pairs))
	for addrStr, amt := range pairs {
		addr, err := paytutil.DecodeAddress(addrStr, chainParams)
		if err != nil {
...
// All errors are returned in paytjson.RPCError format
func sendPairs(w *wallet.Wallet, amounts map[string]paytutil.Amount,
	account uint32, minconf int32, feeSatPerKb paytutil.Amount) (string, error) {
...
func sendFrom(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	...
	amt, err := paytutil.NewAmount(cmd.Amount)
	if err != nil {
		return nil, err
	}
	pairs := map[string]paytutil.Amount{
		cmd.ToAddress: amt,
	}
...
func sendMany(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	...
	pairs := make(map[string]paytutil.Amount, len(cmd.Amounts))
	for k, v := range cmd.Amounts {
		amt, err := paytutil.NewAmount(v)
		if err != nil {
			return nil, err
		}
		pairs[k] = amt
...
func sendToAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.SendToAddressCmd)
...
func signMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.SignMessageCmd)
	...
	wire.WriteVarString(&buf, 0, "payt Signed Message:\n")
	wire.WriteVarString(&buf, 0, cmd.Message)
	messageHash := chainhash.DoubleHashB(buf.Bytes())
	sigbytes, err := payt.SignCompact(payt.S256(), privKey,
		messageHash, true)
	if err != nil {
		return nil, err
	}
...
func signRawTransaction(icmd interface{}, w *wallet.Wallet, chainClient *chain.RPCClient) (interface{}, error) {
	cmd := icmd.(*paytjson.SignRawTransactionCmd)
	...
	var cmdInputs []paytjson.RawTxInput
	if cmd.Inputs != nil {
	...
	for _, rti := range cmdInputs {
		...
		if cmd.PrivKeys != nil && len(*cmd.PrivKeys) != 0 {
			...
			addr, err := paytutil.NewAddressScriptHash(redeemScript,
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	var keys map[string]*paytutil.WIF
	if cmd.PrivKeys != nil {
		keys = make(map[string]*paytutil.WIF)

		for _, key := range *cmd.PrivKeys {
			wif, err := paytutil.DecodeWIF(key)
			if err != nil {
				return nil, DeserializationError{err}
			}
			...
			addr, err := paytutil.NewAddressPubKey(wif.SerializePubKey(),
				w.ChainParams())
			if err != nil {
				return nil, DeserializationError{err}
			}
	...
	signErrors := make([]paytjson.SignRawTransactionError, 0, len(signErrs))
	for _, e := range signErrs {
		input := tx.TxIn[e.InputIndex]
		signErrors = append(signErrors, paytjson.SignRawTransactionError{
	...
	return paytjson.SignRawTransactionResult{
...
func validateAddress(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.ValidateAddressCmd)

	result := paytjson.ValidateAddressWalletResult{}
	addr, err := decodeAddress(cmd.Address, w.ChainParams())
	if err != nil {
...
func verifyMessage(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.VerifyMessageCmd)
	...
	pk, wasCompressed, err := payt.RecoverCompact(payt.S256(), sig,
		expectedMessageHash)
	if err != nil {
		return nil, err
	}
	...
	case *paytutil.AddressPubKeyHash: // ok
		return bytes.Equal(paytutil.Hash160(serializedPubKey), checkAddr.Hash160()[:]), nil
	case *paytutil.AddressPubKey: // ok
...
func walletPassphrase(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.WalletPassphraseCmd)
...
func walletPassphraseChange(icmd interface{}, w *wallet.Wallet) (interface{}, error) {
	cmd := icmd.(*paytjson.WalletPassphraseChangeCmd)
