...
func jsonAuthFail(w http.ResponseWriter) {
	w.Header().Add("WWW-Authenticate", `Basic realm="paytwallet RPC"`)
...
// functional payt wallet RPC server.  This can be called to enable RPC
...
func (s *Server) handlerClosure(request *paytjson.Request) lazyHandler {
	s.handlerMu.Lock()
...
func sanitizeRequest(r *paytjson.Request) string {
...
// but this is only implemented here to work around an oddity with paytjson,
...
func (s *Server) invalidAuth(req *paytjson.Request) bool {
	cmd, err := paytjson.UnmarshalCmd(req)
	if err != nil {
		return false
	}
	authCmd, ok := cmd.(*paytjson.AuthenticateCmd)
	if !ok {
		return false
	}
	// Check credentials.
	...
	for {
		...
		case reqBytes, ok := <-wsc.allRequests:
			...
			default:
				...
				go func() {
					resp, jsonErr := f()
					mresp, err := paytjson.MarshalResponse(req.ID, resp, jsonErr)
					if err != nil {
...
func (s *Server) postClientRPC(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		resp, err := paytjson.MarshalResponse(req.ID, nil, paytjson.ErrRPCInvalidRequest)
		if err != nil {
			log.Errorf("Unable to marshal response: %v", err)
	...
	case "stop":
		stop = true
		res = "paytwallet stopping"
	default:
		res, jsonErr = s.handlerClosure(&req)()
	}

	// Marshal and send.
	mresp, err := paytjson.MarshalResponse(req.ID, res, jsonErr)
	if err != nil {
		log.Errorf("Unable to marshal response: %v", err)
