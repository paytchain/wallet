...
func UseLogger(l paytlog.Logger) {
	grpclog.SetLogger(logger{l})
}

// logger uses a paytlog.Logger to implement the grpclog.Logger interface.
type logger struct {
	paytlog.Logger
}

// stripGrpcPrefix removes the package prefix for all logs made to the grpc
// logger, since these are already included as the paytlog subsystem name.
