...
func TstCreateSeriesCredits(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, seriesID uint32, amounts []int64) []credit {
	...
	for i := range msgTx.TxOut {
		c := wtxmgr.Credit{
			...
			Amount:   paytutil.Amount(msgTx.TxOut[i].Value),
...
func TstNewOutputRequest(t *testing.T, transaction uint32, address string, amount paytutil.Amount,
	net *chaincfg.Params) OutputRequest {
	addr, err := paytutil.DecodeAddress(address, net)
	if err != nil {
...
func TstConstantFee(fee paytutil.Amount) func() paytutil.Amount {
	return func() paytutil.Amount { return fee }
}
...
func createAndFulfillWithdrawalRequests(t *testing.T, dbtx walletdb.ReadWriteTx, pool *Pool, roundID uint32) withdrawalInfo {
	...
	dustThreshold := paytutil.Amount(1e4)
