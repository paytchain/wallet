...
func (p *Pool) DepositScriptAddress(seriesID uint32, branch Branch, index Index) (paytutil.Address, error) {
	script, err := p.DepositScript(seriesID, branch, index)
	if err != nil {
		return nil, err
	}
...
func (p *Pool) addressFor(script []byte) (paytutil.Address, error) {
	scriptHash := paytutil.Hash160(script)
	return paytutil.NewAddressScriptHashFromHash(scriptHash, p.manager.ChainParams())
}
...
func (p *Pool) DepositScript(seriesID uint32, branch Branch, index Index) ([]byte, error) {
	...
	pks := make([]*paytutil.AddressPubKey, len(pubKeys))
	for i, key := range pubKeys {
		...
		pks[i], err = paytutil.NewAddressPubKey(pubkey.SerializeCompressed(),
...
func (p *Pool) addUsedAddr(ns, addrmgrNs walletdb.ReadWriteBucket, seriesID uint32, branch Branch, index Index) error {
	...
	encryptedHash, err := p.manager.Encrypt(waddrmgr.CKTPublic, paytutil.Hash160(script))
	if err != nil {
...
func (p *Pool) getUsedAddr(ns, addrmgrNs walletdb.ReadBucket, seriesID uint32, branch Branch, index Index) (
	...
	addr, err := paytutil.NewAddressScriptHashFromHash(hash, mgr.ChainParams())
	if err != nil {
...
// String returns a string encoding of the underlying payt payment address.
