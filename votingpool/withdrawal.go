...
// feeIncrement is the minimum transation fee (0.00001 payt, measured in nanopayt)
...
// Ntxid is the normalized ID of a given payt transaction, which is generated
...
func (s *WithdrawalStatus) Fees() paytutil.Amount {
	return s.fees
}
...
// Amount returns the amount (in nanopayt) in this OutBailmentOutpoint.
func (o OutBailmentOutpoint) Amount() paytutil.Amount {
	return o.amount
}
...
type withdrawalTx struct {
	...
	// struct field instead of a method so that it can be replaced in tests.
	calculateFee func() paytutil.Amount
}
...
func newWithdrawalTx(setOptions func(tx *withdrawalTx)) *withdrawalTx {
	...
	tx.calculateFee = func() paytutil.Amount {
		return paytutil.Amount(1+tx.calculateSize()/1000) * feeIncrement
	}
	setOptions(tx)
	return tx
}
...
func (tx *withdrawalTx) inputTotal() (total paytutil.Amount) {
...
func (tx *withdrawalTx) outputTotal() (total paytutil.Amount) {
...
// addChange adds a change output if there are any nanopayt left after paying
...
func (p *Pool) StartWithdrawal(ns walletdb.ReadWriteBucket, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	...
	txStore *wtxmgr.Store, txmgrNs walletdb.ReadBucket, chainHeight int32, dustThreshold paytutil.Amount) (
...
func (w *withdrawal) finalizeCurrentTx() error {
	...
	for _, txOut := range tx.outputs {
		...
		amtFulfilled := paytutil.Amount(0)
...
func (w *withdrawal) maybeDropRequests() {
	inputAmount := paytutil.Amount(0)
	for _, input := range w.eligibleInputs {
		inputAmount += input.Amount
	}
	outputAmount := paytutil.Amount(0)
...
func (wi *withdrawalInfo) match(requests []OutputRequest, startAddress WithdrawalAddress,
	lastSeriesID uint32, changeStart ChangeAddress, dustThreshold paytutil.Amount) bool {
...
func getWithdrawalStatus(p *Pool, ns, addrmgrNs walletdb.ReadBucket, roundID uint32, requests []OutputRequest,
	startAddress WithdrawalAddress, lastSeriesID uint32, changeStart ChangeAddress,
	dustThreshold paytutil.Amount) (*WithdrawalStatus, error) {
...
func getRedeemScript(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, addr *paytutil.AddressScriptHash) ([]byte, error) {
...
func signMultiSigUTXO(mgr *waddrmgr.Manager, addrmgrNs walletdb.ReadBucket, tx *wire.MsgTx, idx int, pkScript []byte, sigs []RawSig) error {
	...
	redeemScript, err := getRedeemScript(mgr, addrmgrNs, addresses[0].(*paytutil.AddressScriptHash))
	if err != nil {
