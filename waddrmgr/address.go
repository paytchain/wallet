...
// style of payt addresses.  It additionally provides information about the
// script.
...
// Address returns the paytutil.Address which represents the managed address.
// This will be a pay-to-pubkey-hash address.
//
// This is part of the ManagedAddress interface implementation.
func (a *managedAddress) Address() paytutil.Address {
	return a.address
}
...
func (a *managedAddress) AddrHash() []byte {
	...
	case *paytutil.AddressPubKeyHash:
		hash = n.Hash160()[:]
	case *paytutil.AddressScriptHash:
		hash = n.Hash160()[:]
	case *paytutil.AddressWitnessPubKeyHash:
		hash = n.Hash160()[:]
	}
...
func (a *managedAddress) PubKey() *payt.PublicKey {
	return a.pubKey
}
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) PrivKey() (*payt.PrivateKey, error) {
	...
	privKey, _ := payt.PrivKeyFromBytes(payt.S256(), privKeyCopy)
...
//
// This is part of the ManagedPubKeyAddress interface implementation.
func (a *managedAddress) ExportPrivKey() (*paytutil.WIF, error) {
	pk, err := a.PrivKey()
	if err != nil {
		return nil, err
	}

	return paytutil.NewWIF(pk, a.manager.rootManager.chainParams, a.compressed)
}
...
func newManagedAddressWithoutPrivKey(m *ScopedKeyManager,
	derivationPath DerivationPath, pubKey *payt.PublicKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {

	// Create a pay-to-pubkey-hash address from the public key.
	var pubKeyHash []byte
	if compressed {
		pubKeyHash = paytutil.Hash160(pubKey.SerializeCompressed())
	} else {
		pubKeyHash = paytutil.Hash160(pubKey.SerializeUncompressed())
	}

	var address paytutil.Address
	var err error
	...
	case NestedWitnessPubKey:
		...
		// backwards compatible to payt nodes running 0.6.0 onwards, but
		...
		witAddr, err := paytutil.NewAddressWitnessPubKeyHash(
		...
		address, err = paytutil.NewAddressScriptHash(
	...
	case PubKeyHash:
		address, err = paytutil.NewAddressPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
	...
	case WitnessPubKey:
		address, err = paytutil.NewAddressWitnessPubKeyHash(
			pubKeyHash, m.rootManager.chainParams,
		)
		if err != nil {
			return nil, err
...
func newManagedAddress(s *ScopedKeyManager, derivationPath DerivationPath,
	privKey *payt.PrivateKey, compressed bool,
	addrType AddressType) (*managedAddress, error) {
	...
	ecPubKey := (*payt.PublicKey)(&privKey.PublicKey)
...
type scriptAddress struct {
	manager         *ScopedKeyManager
	account         uint32
	address         *paytutil.AddressScriptHash
...
//
// This is part of the ManagedAddress interface implementation.
func (a *scriptAddress) Address() paytutil.Address {
	return a.address
}
...
func newScriptAddress(m *ScopedKeyManager, account uint32, scriptHash,
	...
	address, err := paytutil.NewAddressScriptHashFromHash(
