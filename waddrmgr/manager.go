...
// would end up being the same payt address (as is the case for
...
func (m *Manager) MarkUsed(ns walletdb.ReadWriteBucket, address paytutil.Address) error {
	m.mtx.RLock()
	defer m.mtx.RUnlock()
...
func (m *Manager) AddrAccount(ns walletdb.ReadBucket,
	address paytutil.Address) (*ScopedKeyManager, uint32, error) {
...
func (m *Manager) ForEachActiveAddress(ns walletdb.ReadBucket, fn func(addr paytutil.Address) error) error {
	m.mtx.RLock()
	defer m.mtx.RUnlock()
