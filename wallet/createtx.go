...
func makeInputSource(eligible []wtxmgr.Credit) txauthor.InputSource {
	...
	currentTotal := paytutil.Amount(0)
	currentInputs := make([]*wire.TxIn, 0, len(eligible))
	currentScripts := make([][]byte, 0, len(eligible))
	currentInputValues := make([]paytutil.Amount, 0, len(eligible))

	return func(target paytutil.Amount) (paytutil.Amount, []*wire.TxIn,
		[]paytutil.Amount, [][]byte, error) {
...
func (s secretSource) GetKey(addr paytutil.Address) (*payt.PrivateKey, bool, error) {
...
func (s secretSource) GetScript(addr paytutil.Address) ([]byte, error) {
	ma, err := s.Address(s.addrmgrNs, addr)
	if err != nil {
		return nil, err
	}
...
func (w *Wallet) txToOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, feeSatPerKb paytutil.Amount, dryRun bool) (
	...
	changeSource := func() ([]byte, error) {
		...
		var changeAddr paytutil.Address
		var err error
	...
	if tx.ChangeIndex >= 0 && account == waddrmgr.ImportedAddrAccount {
		changeAmount := paytutil.Amount(tx.Tx.TxOut[tx.ChangeIndex].Value)
...
func validateMsgTx(tx *wire.MsgTx, prevScripts [][]byte, inputValues []paytutil.Amount) error {
