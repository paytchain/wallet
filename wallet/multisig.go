...
func (w *Wallet) MakeMultiSigScript(addrs []paytutil.Address, nRequired int) ([]byte, error) {
	pubKeys := make([]*paytutil.AddressPubKey, len(addrs))
...
func (w *Wallet) ImportP2SHRedeemScript(script []byte) (*paytutil.AddressScriptHash, error) {
	var p2shAddr *paytutil.AddressScriptHash
	err := walletdb.Update(w.db, func(tx walletdb.ReadWriteTx) error {
		...
		if err != nil {
			...
			if waddrmgr.IsError(err, waddrmgr.ErrDuplicateAddress) {
				...
				p2shAddr, _ = paytutil.NewAddressScriptHash(script,
					w.chainParams)
				return nil
			}
			return err
		...
		p2shAddr = addrInfo.Address().(*paytutil.AddressScriptHash)
		return nil
	})
	return p2shAddr, err
}
