...
// InputSourceError.
type InputSource func(target paytutil.Amount) (total paytutil.Amount, inputs []*wire.TxIn,
	inputValues []paytutil.Amount, scripts [][]byte, err error)
...
type AuthoredTx struct {
	Tx              *wire.MsgTx
	PrevScripts     [][]byte
	PrevInputValues []paytutil.Amount
	TotalInput      paytutil.Amount
...
func NewUnsignedTransaction(outputs []*wire.TxOut, relayFeePerKb paytutil.Amount,
...
func AddAllInputScripts(tx *wire.MsgTx, prevPkScripts [][]byte, inputValues []paytutil.Amount,
...
func spendWitnessKeyHash(txIn *wire.TxIn, pkScript []byte,
	...
	p2wkhAddr, err := paytutil.NewAddressWitnessPubKeyHash(pubKeyHash, chainParams)
	if err != nil {
		return err
	}
