...
func makeInputSource(unspents []*wire.TxOut) InputSource {
	...
	currentInputValues := make([]paytutil.Amount, 0, len(unspents))
	f := func(target paytutil.Amount) (paytutil.Amount, []*wire.TxIn, []paytutil.Amount, [][]byte, error) {
		for currentTotal < target && len(unspents) != 0 {
			u := unspents[0]
			unspents = unspents[1:]
			nextInput := wire.NewTxIn(&wire.OutPoint{}, nil, nil)
			currentTotal += paytutil.Amount(u.Value)
			currentInputs = append(currentInputs, nextInput)
			currentInputValues = append(currentInputValues, paytutil.Amount(u.Value))
		}
...
func TestNewUnsignedTransaction(t *testing.T) {
	tests := []struct {
		...
		Outputs          []*wire.TxOut
		RelayFee         paytutil.Amount
		ChangeAmount     paytutil.Amount
