...
const DefaultRelayFeePerKb paytutil.Amount = 1e3
...
func GetDustThreshold(scriptSize int, relayFeePerKb paytutil.Amount) paytutil.Amount {
	...
	relayFee := paytutil.Amount(totalSize) * byteFee
	return 3 * relayFee
}
...
func IsDustAmount(amount paytutil.Amount, scriptSize int, relayFeePerKb paytutil.Amount) bool {
...
func IsDustOutput(output *wire.TxOut, relayFeePerKb paytutil.Amount) bool {
	...
	return IsDustAmount(paytutil.Amount(output.Value), len(output.PkScript),
		relayFeePerKb)
}
...
// output.
func CheckOutput(output *wire.TxOut, relayFeePerKb paytutil.Amount) error {
	if output.Value < 0 {
		return ErrAmountNegative
	}
	if output.Value > paytutil.MaxNanopayt {
...
func FeeForSerializeSize(relayFeePerKb paytutil.Amount, txSerializeSize int) paytutil.Amount {
	fee := relayFeePerKb * paytutil.Amount(txSerializeSize) / 1000

	if fee == 0 && relayFeePerKb > 0 {
		fee = relayFeePerKb
	}

	if fee < 0 || fee > paytutil.MaxNanopayt {
		fee = paytutil.MaxNanopayt
	}
