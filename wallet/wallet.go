...
// rescan request.
func (w *Wallet) activeData(dbtx walletdb.ReadTx) ([]paytutil.Address, []wtxmgr.Credit, error) {
	addrmgrNs := dbtx.ReadBucket(waddrmgrNamespaceKey)
	txmgrNs := dbtx.ReadBucket(wtxmgrNamespaceKey)

	var addrs []paytutil.Address
	err := w.Manager.ForEachActiveAddress(addrmgrNs, func(addr paytutil.Address) error {
...
func newFilterBlocksRequest(batch []wtxmgr.BlockMeta,
	...
	filterReq := &chain.FilterBlocksRequest{
		Blocks:           batch,
		ExternalAddrs:    make(map[waddrmgr.ScopedIndex]paytutil.Address),
		InternalAddrs:    make(map[waddrmgr.ScopedIndex]paytutil.Address),
...
func (w *Wallet) CreateSimpleTx(account uint32, outputs []*wire.TxOut,
	minconf int32, satPerKb paytutil.Amount, dryRun bool) (
...
// account.
func (w *Wallet) AccountAddresses(account uint32) (addrs []paytutil.Address, err error) {
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// include a UTXO.
func (w *Wallet) CalculateBalance(confirms int32) (paytutil.Amount, error) {
...
type Balances struct {
	Total          paytutil.Amount
	Spendable      paytutil.Amount
	ImmatureReward paytutil.Amount
}
...
func (w *Wallet) CurrentAddress(account uint32, scope waddrmgr.KeyScope) (paytutil.Address, error) {
	...
	if props != nil {
		err = chainClient.NotifyReceived([]paytutil.Address{addr})
		if err != nil {
			return nil, err
		}
...
func (w *Wallet) PubKeyForAddress(a paytutil.Address) (*payt.PublicKey, error) {
	var pubKey *payt.PublicKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
// address.
func (w *Wallet) PrivKeyForAddress(a paytutil.Address) (*payt.PrivateKey, error) {
	var privKey *payt.PrivateKey
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		managedAddr, err := w.Manager.Address(addrmgrNs, a)
		if err != nil {
...
func (w *Wallet) HaveAddress(a paytutil.Address) (bool, error) {
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
...
func (w *Wallet) AccountOfAddress(a paytutil.Address) (uint32, error) {
	var account uint32
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		var err error
...
func (w *Wallet) AddressInfo(a paytutil.Address) (waddrmgr.ManagedAddress, error) {
...
func listTransactions(tx walletdb.ReadTx, details *wtxmgr.TxDetails, addrMgr *waddrmgr.Manager,
	syncHeight int32, net *chaincfg.Params) []paytjson.ListTransactionsResult {
	...
	results := []paytjson.ListTransactionsResult{}
	...
	if len(details.Debits) == len(details.MsgTx.TxIn) {
		var debitTotal paytutil.Amount
		...
		// is calculated.
		feeF64 = (outputTotal - debitTotal).Topayt()
	}
...
outputs:
	for i, output := range details.MsgTx.TxOut {
		...
		amountF64 := paytutil.Amount(output.Value).Topayt()
		result := paytjson.ListTransactionsResult{
			// Fields left zeroed:
			//   InvolvesWatchOnly
			//   BlockIndex
			//
...
func (w *Wallet) ListSinceBlock(start, end, syncHeight int32) ([]paytjson.ListTransactionsResult, error) {
	txList := []paytjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
// replies.
func (w *Wallet) ListTransactions(from, count int) ([]paytjson.ListTransactionsResult, error) {
	txList := []paytjson.ListTransactionsResult{}
...
func (w *Wallet) ListAddressTransactions(pkHashes map[string]struct{}) ([]paytjson.ListTransactionsResult, error) {
	txList := []paytjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		loopDetails:
			for i := range details {
				...
				for _, cred := range detail.Credits {
					...
					apkh, ok := addrs[0].(*paytutil.AddressPubKeyHash)
					if !ok {
						continue
					}
...
// replies.
func (w *Wallet) ListAllTransactions() ([]paytjson.ListTransactionsResult, error) {
	txList := []paytjson.ListTransactionsResult{}
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) Accounts(scope waddrmgr.KeyScope) (*AccountsResult, error) {
	...
	err = walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		m := make(map[uint32]*paytutil.Amount)
...
type AccountBalanceResult struct {
	...
	AccountName    string
	AccountBalance paytutil.Amount
}
...
func (w *Wallet) ListUnspent(minconf, maxconf int32,
	addresses map[string]struct{}) ([]*paytjson.ListUnspentResult, error) {

	var results []*paytjson.ListUnspentResult
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		addrmgrNs := tx.ReadBucket(waddrmgrNamespaceKey)
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
		...
		results = make([]*paytjson.ListUnspentResult, 0, len(unspent))
		for i := range unspent {
			output := unspent[i]
		...
		scSwitch:
			...
			result := &paytjson.ListUnspentResult{
				TxID:          output.OutPoint.Hash.String(),
				Vout:          output.OutPoint.Index,
				Account:       acctName,
				ScriptPubKey:  hex.EncodeToString(output.PkScript),
				Amount:        output.Amount.Topayt(),
...
func (w *Wallet) DumpPrivKeys() ([]string, error) {
	...
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		...
		// privkeys.
		return w.Manager.ForEachActiveAddress(addrmgrNs, func(addr paytutil.Address) error {
...
func (w *Wallet) DumpWIFPrivateKey(addr paytutil.Address) (string, error) {
...
func (w *Wallet) ImportPrivateKey(scope waddrmgr.KeyScope, wif *paytutil.WIF,
	...
	if rescan {
		...
		err := w.chainClient.NotifyReceived([]paytutil.Address{addr})
		if err != nil {
...
func (w *Wallet) LockedOutpoints() []paytjson.TransactionInput {
	locked := make([]paytjson.TransactionInput, len(w.lockedOutpoints))
	i := 0
	for op := range w.lockedOutpoints {
		locked[i] = paytjson.TransactionInput{
			Txid: op.Hash.String(),
			Vout: op.Index,
		}
		i++
...
func (w *Wallet) NewAddress(account uint32,
	scope waddrmgr.KeyScope) (paytutil.Address, error) {
...
func (w *Wallet) newAddress(addrmgrNs walletdb.ReadWriteBucket, account uint32,
	scope waddrmgr.KeyScope) (paytutil.Address, *waddrmgr.AccountProperties, error) {
...
func (w *Wallet) newChangeAddress(addrmgrNs walletdb.ReadWriteBucket,
	account uint32) (paytutil.Address, error) {
...
// returning the total amount of payt received for a single wallet
// address.
func (w *Wallet) TotalReceivedForAddr(addr paytutil.Address, minConf int32) (paytutil.Amount, error) {
	var amount paytutil.Amount
	err := walletdb.View(w.db, func(tx walletdb.ReadTx) error {
		txmgrNs := tx.ReadBucket(wtxmgrNamespaceKey)
...
func (w *Wallet) SendOutputs(outputs []*wire.TxOut, account uint32,
	minconf int32, satPerKb paytutil.Amount) (*wire.MsgTx, error) {
...
func (w *Wallet) SignTransaction(tx *wire.MsgTx, hashType txscript.SigHashType,
	...
	additionalKeysByAddress map[string]*paytutil.WIF,
	...
	err := walletdb.View(w.db, func(dbtx walletdb.ReadTx) error {
		...
		for i, txIn := range tx.TxIn {
			...
			getKey := txscript.KeyClosure(func(addr paytutil.Address) (*payt.PrivateKey, bool, error) {
			...
			getScript := txscript.ScriptClosure(func(addr paytutil.Address) ([]byte, error) {
...
// in order to allow applications wrapping paytwallet to store app-specific data
