...
var (
	...
	TstRecvTx, _                    = paytutil.NewTxFromBytes(TstRecvSerializedTx)
	...
	TstSpendingTx, _           = paytutil.NewTxFromBytes(TstSpendingSerializedTx)
...
func serializeTx(tx *paytutil.Tx) []byte {
...
func TestInsertsCreditsDebitsRollbacks(t *testing.T) {
	...
	dupRecvTx, _ := paytutil.NewTxFromBytes(TstRecvSerializedTx)
	// Switch txout amount to 1 payt.  Transaction store doesn't
	...
	TstDoubleSpendTx := paytutil.NewTx(newDupMsgTx)
	...
	TstSpendingTx := paytutil.NewTx(spendingTx)
	...
	tests := []struct {
		...
		},
		{
			...
			bal: 0,
			unc: paytutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: paytutil.Amount(TstRecvTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstRecvTx.Hash(),
		...
		},
		{
			...
			bal: paytutil.Amount(TstDoubleSpendTx.MsgTx().TxOut[0].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: paytutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
		...
		},
		{
			...
			bal: 0,
			unc: paytutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
					Index: 0,
		...
		},
		{
			...
			bal: paytutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value + TstSpendingTx.MsgTx().TxOut[1].Value),
			unc: 0,
			unspents: map[wire.OutPoint]struct{}{
				wire.OutPoint{
					Hash:  *TstSpendingTx.Hash(),
...
func TestFindingSpentCredits(t *testing.T) {
	...
	expectedBal := paytutil.Amount(TstSpendingTx.MsgTx().TxOut[0].Value)
	if bal != expectedBal {
...
func TestCoinbases(t *testing.T) {
	...
	// Balance should be 0 if the coinbase is immature, 50 payt at and beyond
	// maturity.
	//
...
func TestRemoveUnminedTx(t *testing.T) {
	...
	// balance.
	checkBalance := func(expectedBalance paytutil.Amount,
	...
	checkBalance(paytutil.Amount(initialBalance), false)
	checkBalance(paytutil.Amount(initialBalance), true)
	...
	checkBalance(paytutil.Amount(changeAmount), true)
